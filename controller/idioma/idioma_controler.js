...
const atualizarIdioma = async function (idioma, id, contentType) {
    ...
    try {
        if (String(contentType).toLocaleUpperCase() == 'APPLICATION/JSON') {
            let resultID = await buscarIdioma(id)
...
const validarDados = async function (idioma) {
    ...
    if (idioma.nome == undefined || idioma.nome == null || idioma.nome == '' || idioma.nome.length > 50) {
        message.ERROR_BAD_REQUEST.field = '[Nome] INVÁLIDO'
        return message.ERROR_BAD_REQUEST
    } else if (idioma.sigla == undefined || idioma.sigla == null || idioma.sigla == '' || idioma.sigla.length > 10) {
        message.ERROR_BAD_REQUEST.field = '[Sigla] INVÁLIDA'
        return message.ERROR_BAD_REQUEST
    } else {
        return false
    }
    ...
    try {
        ...
        if (result) {
            if (result.length > 0) {
                ...
                message.DEFAULT_MESSAGE.response.idiomas = result
